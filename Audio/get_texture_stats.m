function out = get_texture_stats(filename)
...
%impose stats whose variables are set to 1
    ...
    P.orig_sound_filename = filename;
    P.orig_sound_folder = '';

    P.output_folder = 'Output_Folder/';

    P.initialize_with_sound_file = 0; %if 1, initializes synthetic sound to a
                                      %waveform in the specified file

    % P.initial_sound_filename = 'white_noise_5s.wav';
    % P.initial_sound_folder = 'Example_Textures/';

    P.write_norm_orig = 1; %save a copy of the original sound file with same rms
                           %as synthetic signal
    P.write_spec_match_noise = 1; %create spectrally matched noise for comparison
